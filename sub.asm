...
section .bss
    ...
    diff resb 6    ; space to store the result (up to 6 digits, including a possible sign and null terminator)
...
convert_to_string:
    ...
    ; Output: diff contains the string representation of the number
    mov edi, diff       ; Point EDI to the result buffer (diff)
    mov ebx, 10         ; Store 10 in EBX (used for division)
    mov ecx, 6          ; Set ECX to the maximum number of digits (including a sign and null terminator)
    add edi, 5          ; Move EDI to the end of the buffer
